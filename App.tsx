...
// Left sidebar (permanent left section)
import LeftSidebar from './components/LeftSidebar';

const PRESETS = [
  { name: 'DuckDuckGo', url: 'duckduckgogg42xjoc72x3sjasowoarfbgcmvfimaftt6twagswzczad.onion' },
  { name: 'Facebook', url: 'facebookwkhpilnemxj7asaniu7vnjjbiltxjqhye3mhbshg7kx5tfyd.onion' },
  { name: 'ProtonMail', url: 'protonmailrmez3lotcc5y737ncbx4yjsc7t52jcnk352jcnk352jcn.onion' },
];
...
function App() {
  ...
  // tool selection state (drishti / falcon)
  const [selectedTool, setSelectedTool] = useState<'drishti' | 'falcon' | null>(null);
  ...
  const handleSearch = async () => {
    // 1. Generate local trace first (checks local DB inside generateRandomTrace)
    const localTrace = generateRandomTrace(query);

    // Check if it was a DB match (based on features tag we added in mockData)
    const isLocalDb = localTrace.features.includes('DatabaseMatch');
    setIsDbMatch(isLocalDb);

    if (isLocalDb) {
      // If found in local DB, use it immediately and skip API call for better "instant" feel
    ...
    // 2. If not in DB, Try API
    try {
      const results = await fetchSessions(1);
      if (results && results.length > 0) {
        const api = results[0];

        // Enhance the random local trace with API mock data
        localTrace.id = `api-${api.session_id}`;
        if (api.target_ip) localTrace.targetQuery = api.target_ip;

        // Minimal overrides
        if (localTrace.nodes.length > 0 && api.suspect_ip) {
          localTrace.nodes[0].ip = api.suspect_ip;
        }

        if (api.confidence_score !== undefined) {
          if (api.confidence_score > 80) localTrace.features.push('API-HighConfidence');
        }
  ...
  // Handler for LeftSidebar selection
  const handleToolSelect = (opt: 'drishti' | 'falcon' | null) => {
    setSelectedTool(opt);
    // placeholder behavior: log selection and you can wire this later
    // when you add Drishti/Falcon functions, call them here
    // e.g., if (opt === 'drishti') openDrishtiPanel();
    // for now we just log and keep state for UI
    // eslint-disable-next-line no-console
    console.log('Tool selected:', opt);
  };

  return (
    <div className="min-h-screen bg-tor-black text-tor-text font-sans pb-12">
      {/* Fixed Left Sidebar */}
      <LeftSidebar selectedTool={selectedTool} onSelect={handleToolSelect} />

      {/* Main content shifted right by sidebar width (260px) */}
      <div style={{ marginLeft: 260 }} className="min-h-screen">
        {/* Modal Overlay */}
        ...
        <main className="max-w-[1600px] mx-auto p-6 space-y-6">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-blue-400 font-mono text-xl">
                      sess_{currentTrace?.id.substring(0, 12) || '---'}
                    </div>
                    {isDbMatch && (
                      <span className="text-[10px] text-tor-green border border-tor-green/50 px-1.5 py-0.5 rounded bg-tor-green/10">VERIFIED DATABASE HIT</span>
                    )}
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-white font-mono text-xl">
                    0ms <span className="text-gray-600 text-sm">/ 3395ms</span>
                  </div>
                </div>
              </div>
            ...
            <div className="col-span-4 bg-tor-panel border border-tor-border rounded p-4 flex flex-col items-center justify-center relative overflow-hidden">
              ...
              <div
                className={`text-6xl font-bold ${
                  (currentTrace?.anomalies.length || 0) > 1
                    ? 'text-tor-red drop-shadow-[0_0_15px_rgba(255,42,42,0.5)]'
                    : (currentTrace?.anomalies.length || 0) > 0
                    ? 'text-tor-yellow'
                    : 'text-tor-green'
                }`}
              >
          ...
          <div className="flex border-b border-tor-border">
            ...
            <div className="ml-auto flex items-center gap-4 pr-4 text-[10px] font-mono text-gray-500 uppercase">
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-white" /> Source
              </div>
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-tor-green" /> Guard Node
              </div>
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-tor-blue" /> Middle Relay
              </div>
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-tor-red" /> Exit Node
              </div>
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-tor-yellow" /> Destination
              </div>
            </div>
          </div>

          {/* --- MAIN VISUALIZATION AREA --- */}
          <div className="grid grid-cols-12 gap-6 min-h-[420px] max-h-[460px]">
            {/* Network Graph / Map */}
            ...
            <div className="col-span-5 h-full flex flex-col gap-4">
              ...
              <div className="bg-tor-panel border border-tor-border rounded-lg p-4 font-mono text-xs flex-1 min-h-[140px]">
                ...
                {currentTrace && currentTrace.packets.length > 0 &&
                  currentTrace.packets[
                    ...
                    <div className="grid grid-cols-2 gap-y-4">
                      ...
                      <div>
                        ...
                        >
                          {currentTrace.targetQuery.length > 25
                            ? currentTrace.targetQuery.substring(0, 25) + '...'
          ...
          {/* --- AI ANALYSIS --- */}
          {currentTrace && <AnalysisPanel trace={currentTrace} />}
